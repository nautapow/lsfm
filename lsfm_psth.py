...
def psth_wwo_bf(resp, para, bf, version, filename, bandwidth=None, plot=False, saveplot=False):
    """
    ...
    if bandwidth:
        resp_in, resp_ex, para_in, para_ex, _, _ = lsfm.resp_bfband_or_not(resp, para, bf, bandwidth)
    else:
        resp_in, resp_ex, para_in, para_ex, _, _ = lsfm.resp_bf_or_not(resp, para, bf)
...
def get_section(psth_sep, para_sep, para_type: int):
    """
...
def plot_category(filename, bf, data, para_type: int, saveplot=False, **kwargs):
    ...
    **kwargs : TYPE
    ...
    """
    p_type = ['cf', 'bw', 'mr']
    titles = ['center frequency', 'bandwidth', 'modulation rate']
    ...
    for idx, (key, value) in enumerate(data.items()):
        if key != 'parameter' or 'repeat':
            ...
            fig, ax = plt.subplots()
            ax.scatter(xx,yy)
            ax.set_title(f'{key}-cf', fontsize=16)
            ax.set_xlabel(f'{x_labels[para_type]}', fontsize=14)
            ax.set_ylabel(f'{y_labels[idx]}', fontsize=14)
            
            if saveplot:
                plt.savefig(f'{filename}_{p_type[para_type]}_{key}.png', dpi=500, bbox_inches='tight')
            plt.show()
            plt.close()
...
def plot_single(filename, bf, psth_sep, psth_section, para_type: int, saveplot=False, **kwargs):
    ...
    for idx, para in enumerate(paras):
        ...
        [ax.axvline(x=_x, color='k', linestyle='--', alpha=0.5) for _x in [1250,26250]]
        ax.set_title(f'{filename}  {p_type} {para}', fontsize=16)
        if saveplot:
